...
class LogsDataProcessor:
    ...
    def _load_df(self, sort_temporally = False):
        ...
        # D: Adjust to format = mixed
        #df["time:timestamp"]= pd.to_datetime(df["time:timestamp"],  
        #    dayfirst=True).map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"))
        df["time:timestamp"]= pd.to_datetime(df["time:timestamp"],  
            format="mixed").map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def _next_activity_helper_func(self, df):
        ...
        for _, case in enumerate(unique_cases):
            print(_, case)  # Debugging print statement to track progress
    ...
    def __remaining_time_helper_func(self, df):
        case_id = "case:concept:name"
        event_name = "concept:name"
        event_time = "time:timestamp"
        processed_df = pd.DataFrame(columns = ["case_id", "prefix", "k", "time_passed", 
    ...
    def _remaining_time_helper_func(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df[[case_id, event_name, event_time]].copy()
        df = df.dropna(subset=[case_id])
        df[event_time] = pd.to_datetime(df[event_time].astype(str).str[:19], format="%Y-%m-%d %H:%M:%S")
        df = df.sort_values([case_id, event_time])

        df["latest_time"] = df.groupby(case_id)[event_time].diff().dt.days.fillna(0).astype(int)
        df["recent_time"] = df.groupby(case_id)[event_time].diff(2).dt.days.fillna(0).astype(int)
        df["time_passed"] = df.groupby(case_id)["latest_time"].cumsum()
        df["last_time"] = df.groupby(case_id)[event_time].transform("max")
        df["remaining_time_days"] = (df["last_time"] - df[event_time]).dt.days.astype(int)
        df["k"] = df.groupby(case_id).cumcount()

        # Prefix still needs a per-case loop
        prefix_values = []
        for _, case_df in df.groupby(case_id, sort=False):
            print(_, case_id)
            acts = case_df[event_name].tolist()
            prefix = ""
            for i, act in enumerate(acts):
                prefix = act if i == 0 else prefix + " " + act
                prefix_values.append(prefix)

        df["prefix"] = prefix_values

        return df[[
            case_id, "prefix", "k", "time_passed",
            "recent_time", "latest_time", "remaining_time_days"
        ]].rename(columns={case_id: "case_id"})
    ...
    def _process_remaining_time(self, df, train_list, test_list):
        ...
        # D: Changed to use temporal splitting - directly induced on train and test DF
        train_remaining_time = self._remaining_time_helper_func(train_list)
        test_remaining_time = self._remaining_time_helper_func(test_list)
    ...
    # D: Added split timestamp for temporal splitting
    def process_logs(self, task, sort_temporally = False, train_test_ratio = 0.80, split_timestamp = None, last_timestamp = None, only_full_traces = True):
        ...
        # ----- D: Following added for temporal split
        split_timestamp = pd.to_datetime(split_timestamp, utc=True)
        df["time:timestamp_new"] = pd.to_datetime(df["time:timestamp"], utc=True)
        if only_full_traces:
            full_cases = (
                df.groupby("case:concept:name")["time:timestamp_new"]
                .max()
                .loc[lambda x: x <= split_timestamp]
                .index
                .to_numpy()
            )

            train_df = df[df["case:concept:name"].isin(full_cases)]
        else:
            train_df = df[df["time:timestamp_new"] <= split_timestamp]

        test_df = df[df["time:timestamp_new"] <= split_timestamp]
        rest = df[df["time:timestamp_new"] > split_timestamp]
        if last_timestamp != None:
            rest = rest[rest["time:timestamp_new"] <= last_timestamp]
        case_ids_rest = rest["case:concept:name"].unique()
        case_ids_test = test_df["case:concept:name"].unique()
        missing = case_ids_rest[~np.isin(case_ids_rest, case_ids_test)] # All case ids that are only in rest
        row_list = []
        for id in missing:
            # Get data
            c = df[df["case:concept:name"] == id]
            if c.empty:
                print(f"Empty case selection for case_id={id!r}")
                continue
            # Get earliest event
            row = c.loc[c["time:timestamp_new"].idxmin()]
            # Attach row to train set
            row_list.append(row)
        train_df = pd.concat([test_df, pd.DataFrame(row_list)], ignore_index=True)
        ...
        if task == Task.NEXT_ACTIVITY:
            ...
            self._process_remaining_time(df, train_df, test_df)
